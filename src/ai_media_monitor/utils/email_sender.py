...
class EmailSender:
    ...
    def _generate_html_content(
        self, report_data: dict[str, Any], markdown_content: str = None
    ) -> str:
        ...
        if markdown_content:
            ...
            main_content_html = (
                f"{highlights_html}{trending_topics_html}{experts_html}{suggestions_html}"
            )
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 800px; margin: 0 auto; background-color: #f5f5f5;">
    ...
    def _generate_highlights_html(self, highlights: dict[str, Any]) -> str:
        ...
        if top_topic:
            ...
            html.append(
                f"<p><strong>Sentiment:</strong> {top_topic.get('sentiment', 'neutral')}</p>"
            )
            html.append(
                f"<p><strong>Invalshoek:</strong> {top_topic.get('suggested_angle', 'N/A')}</p>"
            )
            html.append("</div>")
        ...
        if top_expert:
            ...
            html.append(
                f"<p><strong>Recente quotes:</strong> {top_expert.get('recent_quotes', 0)}</p>"
            )
            html.append("</div>")
    ...
    def _generate_suggestions_html(self, suggestions: list[dict[str, Any]]) -> str:
        ...
        for i, suggestion in enumerate(suggestions[:5], 1):
            ...
            html.append(
                f"<p><strong>Relevantie Score:</strong> {suggestion.get('relevance_score', 0):.1f}/10</p>"
            )
            html.append(f"<p><strong>Reden:</strong> {suggestion.get('reason', 'N/A')}</p>")
            html.append(
                f"<p><strong>Unieke Invalshoek:</strong> {suggestion.get('unique_angle', 'N/A')}</p>"
            )
    ...
    async def send_report_email(
        ...
        try:
            ...
            msg["Subject"] = subject
            msg["From"] = (
                f"{self.config['email']['from_name']} <{self.config['email']['from_email']}>"
            )
            ...
            if (
                attachments_config.get("include_markdown", True)
                and markdown_file
            ...
            with smtplib.SMTP(smtp_config["server"], smtp_config["port"]) as server:
                ...
                text = msg.as_string()
                server.sendmail(smtp_config["username"], [self.config["email"]["to_email"]], text)
